...
class inpaint_image:
    ...
    def confidence_updater(self):
        ...
        for point in required_front:
            ...
            confidence_updates[point[0], point[1]] = sum(
                sum(self.patch_on_source(self.confidence, patch))) / self.area_patch(patch)
    ...
    def update_image(self, target_pixel, source_patch):
        ...
        target_patch = self.patch_around_point(target_pixel)
        pixels_positions = np.argwhere(self.patch_on_source(self.working_mask, target_patch) == 1) + [
...
image = io.imread("tree1_region_1.jpg")
...
output_image = inpaint_image(image, mask, 7).inpainter()
plt.imshow(output_image)
plt.xticks([])
plt.yticks([])
plt.show()
# io.imsave("restored.jpg", output_image)
cv2.imwrite("restored.jpg", output_image[:, :, (2, 1, 0)])
