...
if __name__ == '__main__':
    ...
    filename = '../image/circle/circle.jpg'
    ...
    while (1):
        ...
        if k == 27:  # esc to exit
            ...
            print(" mark pro_background regions with left mouse button \n")
            value = DRAW_PR_BG
        elif k == ord('3'):  # PR_FG drawing
            print(" mark pro_foreground regions with left mouse button \n")
            value = DRAW_PR_FG
        elif k == ord('s'):  # save image
            bar = np.zeros((img.shape[0], 5, 3), np.uint8)
            res = np.hstack((img2, bar, img, bar, output))
            cv.imwrite('grabcut_output.jpg', res)
